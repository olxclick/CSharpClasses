...
namespace Database
{
	class Program
	{
		...
		{
			DbConnection sqlConnection = new SqlConnection("SQL Connection String", TimeSpan.FromSeconds(10));
			sqlConnection.Open();
			sqlConnection.Close();

			DbConnection oracleConnection = new OracleConnection("Oracle Connection String", TimeSpan.FromSeconds(15));
